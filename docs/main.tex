...
However, only the rule-based method follows the M/M/1 queueing model closely, while the embedding and LLM methods deviate from the model at higher utilization levels.
...
%     \caption{Measured vs.\ predicted mean response time \(W\) as a function of utilization \(\rho\). The solid blue line is the M/M/1 prediction using the median \(\mu\) for each technique. Red points are measured values with 95\% bootstrap confidence intervals.}
%     \label{fig:short_w_vs_rho}
% \end{figure}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../results/analysis/short/throughput.png}
    \caption{Throughput (requests per second) as a function of sending rate. The server saturates at the maximum throughput for each moderation technique.}
...
\begin{figure}[htbp]
    ...
    \caption{Observed inter-arrival times vs.\ theoretical exponential quantiles, validating the Poisson arrival assumption.}
...
The results for the second experiment are shown in the following table and plots.
...
There are two main questions to answer, one concerning the modeling and the other is about which method is the best for chat moderation.

The first question to answer is: \textit{How does the throughput scale with the arrival rate \(\lambda\) for each moderation technique, and at what utilization \(\rho\) does the system begin to saturate?}

I hypothesize that:
\begin{itemize}
    \item The \textbf{rule-based} endpoint (pure CPU, near-deterministic service times \(\sigma \approx 0.2\)~ms) will fit M/M/1 well, despite its low variance, because utilization is the dominant factor.
...
A controlled-rate benchmark client was implemented to generate Poisson arrivals with a target rate \(\lambda\) (requests per second). For each technique, I collected 100 samples at five different arrival rates spanning from less than half saturation to over the saturation point.
...
Table~\ref{tab:mm1} summarizes the measured parameters and model predictions.
...
The rule-based endpoint matches the M/M/1 model closely across all utilization levels (error \(<5\%\)). Although its service times are nearly deterministic (\(\text{CV} \ll 1\)), the M/M/1 prediction remains accurate because the queueing dynamics are dominated by arrival variability at moderate-to-high \(\rho\).

The embedding model shows good agreement at low \(\rho\) but begins deviating above \(\rho \approx 0.6\).
...
If we were to refine the model, the \textbf{M/G/1} queue (Pollaczek--Khinchine formula) would be a better fit for the embedding and LLM endpoints, as it only requires the mean and variance of the service time without assuming a specific distribution.
